...
class DropTheNumber extends Game with TapDetector {
    ...
    **********************************************************************/
    void setupCurrentBlock() {
        ...
        currentBlock = Block(nextBlockValue, 15, 30); /////////////////////////////////////temporary set x and y for debug!
        nextBlockValue = pow(2, random.nextInt(MAX_POWER)+POWER_OFFSET).toInt();
    }
    ...
    **********************************************************************/
    void togglePause() {
        if (isGameRunning()) {
            pause = !pause;
            if(!pause) {
                pauseElapsedTime = DateTime.now().difference(startTime.add(elapsedTime));
            }
        }
    }
